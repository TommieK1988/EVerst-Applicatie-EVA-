...
export function getStatusKleur(status: string): string {
  const map: Record<string, string> = {
    ...
    gewonnen: 'bg-everts-50 text-everts-dark border-everts-200',
    verloren: 'bg-red-50 text-red-600 border-red-200',
    uitvoering: 'bg-amber-50 text-amber-700 border-amber-200',
    afgerond: 'bg-slate-50 text-slate-500 border-slate-200',
  }
