/**
 * EvertsCalc Local Store
 * Slaat alle calculatiedata op in localStorage.
 * Valt terug op mock-data als er nog niets is opgeslagen.
 */
...
import {
  mockProjecten, mockDeelprojecten, mockLocaties, mockElementen,
  mockActiviteiten, mockLijnen, mockScenarios, mockBibliotheek,
  mockGroepen, mockCalculatieregels, mockComponentregels,
} from './mock-data'
...
export function getProjectVolgnummer(): number {
  ...
  return parseInt(localStorage.getItem(KEYS.projectvolgnummer) || '3', 10)
}
...
export function getProjecten(): Project[] {
  return lees(KEYS.projecten, mockProjecten)
}
...
export function getDeelprojecten(project_id?: string): Deelproject[] {
  const alle = lees(KEYS.deelprojecten, mockDeelprojecten)
...
export function getLocaties(deelproject_id?: string): Locatie[] {
  const alle = lees(KEYS.locaties, mockLocaties)
...
export function getElementen(locatie_id?: string): Element[] {
  const alle = lees(KEYS.elementen, mockElementen)
...
export function getScenarios(project_id?: string): Scenario[] {
  const alle = lees(KEYS.scenarios, mockScenarios)
...
export function getActiviteiten(element_id?: string, scenario_id?: string): Activiteit[] {
  const alle = lees(KEYS.activiteiten, mockActiviteiten)
...
export function getLijnen(activiteit_id?: string): CalculatieLijn[] {
  const alle = lees(KEYS.lijnen, mockLijnen)
...
export function getBibliotheek(discipline?: string): BibliotheekActiviteit[] {
  const alle = lees(KEYS.bibliotheek, mockBibliotheek)
...
export function getGroepen(scenario_id?: string): Groep[] {
  const alle = lees(KEYS.groepen, mockGroepen)
...
export function getCalculatieregels(groep_id?: string): Calculatieregel[] {
  const alle = lees(KEYS.calculatieregels, mockCalculatieregels)
...
export function getComponentregels(calculatieregel_id?: string): Componentregel[] {
  const alle = lees(KEYS.componentregels, mockComponentregels)
