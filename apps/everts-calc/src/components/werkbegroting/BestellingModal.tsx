...
import { slaBestellingOp } from '@/lib/local-store'
import type { WerkbegrotingComponent, WerkbegrotingBestelling } from '@/lib/types'
              ...
              {componenten.map(c => (
                <div key={c.id} className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-mono text-slate-500 flex-shrink-0">
                    {formatEuro(c.norm_hoeveelheid * c.tarief)}
                  </span>
                </div>
              ))}
            </div>
