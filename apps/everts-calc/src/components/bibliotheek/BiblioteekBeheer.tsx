...
import { Search, BookOpen, Pencil, Trash2, Plus, Upload, X, Loader2, ChevronDown, Download } from 'lucide-react'
...
} from '@/app/actions/bibliotheek'
import { formatEuro } from '@/lib/calculations'
import { getInstellingen, getMaterialen } from '@/lib/local-store'
import type { PaintItemMetNormen, LaborNorm, MaterialNorm } from '@/lib/services/bibliotheek'
import type { Materiaal } from '@/lib/types'
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center p-6">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-8 py-5 border-b border-slate-200 sticky top-0 bg-white z-10 rounded-t-2xl">
          <div>
          ...
          <button onClick={onSluiten} className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 p-8 space-y-6">
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setNwLaborOpen(o => !o)}
                className="flex items-center gap-1 text-xs text-everts hover:text-everts-dark font-medium">
            ...
            {nwLaborOpen && (
              <div className="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200 space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={voegLaborToe}
                    className="px-3 py-1.5 bg-everts text-white text-xs rounded-lg font-medium hover:bg-everts-dark">
                    Opslaan
                  </button>
                  <button onClick={() => setNwLaborOpen(false)}
                    className="px-3 py-1.5 text-slate-500 text-xs rounded-lg hover:bg-slate-200">
                    Annuleren
                  </button>
                </div>
              </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setNwMatOpen(o => !o)}
                className="flex items-center gap-1 text-xs text-everts hover:text-everts-dark font-medium">
                <Plus className="w-3.5 h-3.5" /> Toevoegen
              </button>
            </div>
            ...
            {nwMatOpen && (
              <div className="mt-3 p-3 bg-red-50 rounded-lg border border-red-200 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setNwMatOpen(false)}
                    className="px-3 py-1.5 text-slate-500 text-xs rounded-lg hover:bg-slate-200">
                    Annuleren
                  </button>
                </div>
          ...
          <section>
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setNwOaOpen(o => !o)}
                className="flex items-center gap-1 text-xs text-everts hover:text-everts-dark font-medium">
                <Plus className="w-3.5 h-3.5" /> Toevoegen
              </button>
            </div>
            ...
            {nwOaOpen && (
              <div className="mt-3 p-3 bg-purple-50 rounded-lg border border-purple-200 space-y-2">
                ...
                <div className="flex gap-2">
                  <button onClick={voegOaToe}
                    className="px-3 py-1.5 bg-everts text-white text-xs rounded-lg font-medium hover:bg-everts-dark">
                    Opslaan
                  </button>
                  <button onClick={() => setNwOaOpen(false)}
                    className="px-3 py-1.5 text-slate-500 text-xs rounded-lg hover:bg-slate-200">
                    Annuleren
                  </button>
                </div>
        ...
        {/* Footer: Opslaan */}
        <div className="sticky bottom-0 bg-white border-t border-slate-200 px-8 py-4 flex justify-end gap-3 rounded-b-2xl">
          <button onClick={onSluiten}
            className="px-4 py-2 text-sm text-slate-600 hover:bg-slate-100 rounded-lg transition-colors">
            Sluiten
          </button>
          <button onClick={slaBasiOp} disabled={isPending}
            className="px-5 py-2 text-sm font-semibold bg-everts hover:bg-everts-dark disabled:opacity-60 text-white rounded-lg transition-colors flex items-center gap-2">
...
function NieuwReceptModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
      <div className="absolute inset-0 bg-black/40" onClick={onSluiten} />
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8 space-y-5">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-slate-800">Nieuw recept</h2>
          <button onClick={onSluiten} className="p-1 text-slate-400 hover:text-slate-600">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          <button onClick={maak}
            className="flex-1 bg-everts hover:bg-everts-dark text-white font-semibold py-2.5 rounded-lg text-sm transition-colors">
            Aanmaken
          </button>
          <button onClick={onSluiten}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          <a
            href="/api/bibliotheek/sjabloon"
            download="import-sjabloon.xlsx"
            className="flex items-center gap-2 px-4 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-600 hover:bg-slate-50 whitespace-nowrap"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-600 hover:bg-slate-50 disabled:opacity-50 whitespace-nowrap"
          >
            {importBezig
              ? <><Loader2 className="w-4 h-4 animate-spin" /> Importeren...</>
              : <><Upload className="w-4 h-4" /> Importeer Excel</>
            }
          </button>
          <button
            onClick={handleLeegMaak}
            className="flex items-center gap-2 px-4 py-2.5 border border-red-200 text-red-500 hover:bg-red-50 rounded-lg text-sm font-medium whitespace-nowrap transition-colors"
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-everts hover:bg-everts-dark text-white rounded-lg text-sm font-medium whitespace-nowrap"
          >
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                ...
                <tr key={item.id}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => setBewerkItemId(item.id)}
                        className="p-1.5 text-slate-400 hover:text-everts hover:bg-everts/10 rounded-lg"
                        title="Bewerken"
                      >
        ...
        {gefilterd.length === 0 && (
          <div className="py-16 text-center text-slate-400">
            ...
            <p className="font-medium">Geen recepten gevonden</p>
            {zoek && <p className="text-sm mt-1">Probeer een andere zoekterm</p>}
          </div>
        )}
      </div>
