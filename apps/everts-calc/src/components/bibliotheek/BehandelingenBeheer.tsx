...
} from '@/app/actions/behandelingen'
import type { DbPaintTreatment } from '@/lib/supabase/database.types'
      ...
      <tr className="bg-everts/5">
        ...
        <td className="px-4 py-2">
          <div className="flex items-center justify-end gap-1">
            <button onClick={sla}
              className="p-1.5 text-white bg-everts hover:bg-everts-dark rounded-lg"
              title="Opslaan">
    ...
    <tr className={`group hover:bg-slate-50 transition-colors ${!behandeling.active ? 'opacity-40' : ''}`}>
      ...
      <td className="px-4 py-3">
        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => setBewerkModus(true)}
          ...
          <button onClick={toggle}
            className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg"
            title={behandeling.active ? 'Deactiveren' : 'Activeren'}>
    ...
    <div className="space-y-5">
      ...
      {nieuwOpen && (
        <div className="bg-everts/5 border border-everts/20 rounded-xl p-4 flex gap-3 items-end">
          ...
          <button onClick={maak}
            className="px-4 py-2 bg-everts hover:bg-everts-dark text-white text-sm font-medium rounded-lg whitespace-nowrap">
            Aanmaken
          </button>
          <button onClick={() => setNieuwOpen(false)}
            className="px-4 py-2 border border-slate-300 text-slate-600 text-sm rounded-lg hover:bg-slate-50 whitespace-nowrap">
            Annuleren
          </button>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        {gefilterd.length === 0 && (
          <div className="py-16 text-center text-slate-400">
            <p className="font-medium">Geen behandelingen gevonden</p>
