...
import { Plus, Trash2, Download, Upload, Search, Loader2, X } from 'lucide-react'
import { getMaterialen, slaMateriaalOp, verwijderMateriaal } from '@/lib/local-store'
import { nieuweId } from '@/lib/utils'
import type { Materiaal, MateriaalStatus } from '@/lib/types'
import { MATERIAAL_GROEPEN } from '@/lib/types'
  ...
  return (
    <tr className={`group border-b border-slate-100 hover:bg-slate-50/80 transition-colors ${isInactief ? 'opacity-50' : ''}`}>
      ...
      <td className="px-1 py-0.5 w-8 text-center">
        <button
          onClick={onVerwijder}
          className="p-0.5 text-slate-200 opacity-0 group-hover:opacity-100 hover:text-red-400 rounded transition-all"
...
export default function MaterialenBibliotheek() {
  ...
  const downloadSjabloon = () => {
    window.location.href = '/api/materialen/sjabloon'
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-shrink-0 px-4 py-3 bg-white border-b border-slate-200 flex items-center gap-3 flex-wrap">
        ...
        <div className="relative flex-1 min-w-48">
          ...
          />
          {zoek && (
            <button onClick={() => setZoek('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500">
        ...
        <div className="flex items-center gap-2 ml-auto">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-lg text-xs"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 text-slate-600 hover:bg-slate-50 rounded-lg text-xs disabled:opacity-50"
          ...
          <button
            onClick={voegToe}
            className="flex items-center gap-1.5 px-3 py-2 bg-everts text-white hover:bg-everts/90 rounded-lg text-xs font-medium"
          >
      ...
      {/* ─── Import resultaat ─────────────────────────────────────────────── */}
      {importResultaat && (
        <div className={`flex-shrink-0 px-4 py-2 text-xs border-b flex items-center gap-3 ${
          importResultaat.fouten.length > 0 ? 'bg-amber-50 border-amber-200' : 'bg-green-50 border-green-200'
        }`}>
          <span className={importResultaat.fouten.length > 0 ? 'text-amber-700' : 'text-green-700'}>
          ...
          {importResultaat.fouten.slice(0, 2).map((f, i) => (
            <span key={i} className="text-red-600">{f}</span>
          ))}
          <button onClick={() => setImportResultaat(null)} className="ml-auto text-slate-400 hover:text-slate-600">
            <X className="w-3.5 h-3.5" />
          </button>
        </div>
      )}
        ...
        {gefilterd.length === 0 && (
          <div className="py-20 text-center text-slate-400">
            <p className="text-sm font-medium">
              {materialen.length === 0 ? 'Nog geen materialen' : 'Geen materialen gevonden'}
            </p>
            {materialen.length === 0 && (
              <p className="text-xs mt-1">Klik op "Materiaal toevoegen" om te beginnen, of importeer een Excel-bestand.</p>
            )}
            {materialen.length > 0 && (
              <p className="text-xs mt-1">Pas de filters of zoekterm aan</p>
            )}
          </div>
        )}
      </div>
