...
import {
  ChevronRight, ChevronDown, Plus, Trash2, Loader2, Check, X, Upload, Download, Pencil, Search,
} from 'lucide-react'
import type { SchilderOnderdeel, SchilderType, SchilderBehandeling, SchilderCombinatie } from '@/lib/services/schilderwerk'
import {
  haalCombinaties,
...
} from '@/app/actions/schilderwerk'
import { valideerFormule, formuleTekst } from '@/lib/schilder-formule'
  ...
  return (
    <div className="px-3 py-2 space-y-1.5 border-b border-slate-100 bg-slate-50/50">
      ...
      <div className="flex gap-1.5 justify-end">
        <button onClick={submit} className="px-2 py-0.5 text-xs bg-everts text-white rounded hover:bg-everts/90">Toevoegen</button>
        <button onClick={onAnnuleer} className="px-2 py-0.5 text-xs text-slate-500 border border-slate-200 rounded hover:bg-white">Annuleren</button>
      </div>
    </div>
  )
}
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 flex items-center gap-3 px-4 py-3 bg-white border-b border-slate-200">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 bg-everts text-white rounded-lg text-sm hover:bg-everts/90 whitespace-nowrap"
        >
      ...
      {addOpen && (
        <div className="flex-shrink-0 px-4 py-3 bg-everts/5 border-b border-everts/20 flex items-end gap-3 flex-wrap">
          ...
          <button onClick={handleAdd} className="px-3 py-1.5 text-xs bg-everts text-white rounded hover:bg-everts/90">Aanmaken</button>
          <button onClick={() => setAddOpen(false)} className="px-3 py-1.5 text-xs text-slate-500 border border-slate-200 rounded hover:bg-white">Annuleren</button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-auto">
        ...
        {gefilterd.length === 0 && (
          <div className="py-16 text-center text-slate-400">
            <p className="text-sm font-medium">Geen behandelingen gevonden</p>
            {zoek && <p className="text-xs mt-1">Probeer een andere zoekterm</p>}
            {!zoek && (
              <p className="text-xs mt-1">Voeg een behandeling toe via de knop rechtsbovenin</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="w-72 flex-shrink-0 border-r border-slate-200 bg-white flex flex-col overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            {/* Sjabloon downloaden */}
            <a href="/api/schilderwerk/sjabloon" download>
              <button
                type="button"
            ...
            >
              {importBezig ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Upload className="w-3.5 h-3.5" />}
            </button>
            <button
        ...
        {/* Import resultaat */}
        {importResultaat && (
          <div className={`px-3 py-2 text-xs border-b ${importResultaat.fouten.length > 0 ? 'bg-amber-50 border-amber-200' : 'bg-green-50 border-green-200'}`}>
            <p className={importResultaat.fouten.length > 0 ? 'text-amber-700' : 'text-green-700'}>
              ✓ {importResultaat.aangemaakt} aangemaakt, {importResultaat.bijgewerkt} bijgewerkt
            </p>
            {importResultaat.fouten.slice(0, 3).map((f, i) => (
              <p key={i} className="text-red-600 mt-0.5">{f}</p>
            ))}
            {importResultaat.fouten.length > 3 && <p className="text-red-500">... en {importResultaat.fouten.length - 3} meer fouten</p>}
            <button onClick={() => setImportResultaat(null)} className="text-slate-400 mt-1">✕ sluiten</button>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto py-1">
          ...
          {onderdelen.length === 0 && (
            <p className="text-xs text-slate-400 text-center py-8">Nog geen onderdelen</p>
          )}
        ...
        {!selectedTypeId ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center text-slate-400">
              <p className="text-sm font-medium">Selecteer een type links</p>
              <p className="text-xs mt-1">om de schilderrecepten te beheren</p>
            </div>
          </div>
        ) : (
          <>
            <div className="flex-shrink-0 px-6 py-3 bg-white border-b border-slate-200 flex items-center justify-between">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg bg-everts text-white hover:bg-everts/90 transition-colors"
              >
                <Plus className="w-3.5 h-3.5" />
            ...
            {showAddRecept && (
              <div className="flex-shrink-0 px-6 py-3 bg-blue-50 border-b border-blue-100">
                ...
                <div className="flex gap-2 items-end flex-wrap">
                  ...
                  <button onClick={handleAddRecept} className="px-3 py-1.5 text-xs bg-everts text-white rounded hover:bg-everts/90 flex-shrink-0">Toevoegen</button>
                  <button onClick={() => { setShowAddRecept(false); setAddReceptBehandelingId(''); setAddNieuwBehandeling('') }} className="px-2 py-1.5 text-xs text-slate-500 border border-slate-200 rounded hover:bg-white flex-shrink-0">Annuleren</button>
                </div>
              </div>
            )}
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
              ...
              {!isLoading && combinaties.length === 0 && (
                <div className="text-center py-12 text-slate-400">
                  <p className="text-sm font-medium">Geen schilderrecepten</p>
                  <p className="text-xs mt-1">Voeg een behandeling toe via de knop rechtsbovenin</p>
                </div>
              )}
