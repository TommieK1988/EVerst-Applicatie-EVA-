...
import { getInstellingen, slaInstellingenOp } from '@/lib/local-store'
...
export default function Instellingen() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <PageHeader
        title="Instellingen"
        description="Standaard tarieven en opslagen"
      />
...
function CalculatieTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Uurtarieven — beheerd via EVA */}
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
        <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
          <Calculator className="w-4 h-4 text-blue-600" />
        </div>
        <div>
          <div className="text-sm font-semibold text-blue-900 mb-0.5">Uursoorten &amp; uurtarieven</div>
          <div className="text-xs text-blue-700">
            Uurtarieven en uursoorten worden beheerd via EVA → Instellingen → Planning.
          </div>
        </div>
      </div>
      ...
      {/* Standaard kostengroepen */}
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <h3 className="font-semibold text-slate-800 mb-1">Standaard kostengroepen</h3>
        <p className="text-xs text-slate-400 mb-4">
          Kostengroepen worden gebruikt in het rekenblad en de werkbegroting om kostenregels te groeperen (bijv. Bouwplaats, Bereikbaarheid).
        </p>
        <div className="flex flex-wrap gap-2 mb-3">
          {kostengroepen.map(k => (
            <div key={k} className="inline-flex items-center gap-1 bg-everts-50 border border-everts/20 rounded-lg px-2.5 py-1">
              <span className="text-sm text-everts font-medium">{k}</span>
              <button onClick={() => setKostengroepen(prev => prev.filter(x => x !== k))} className="text-everts/40 hover:text-red-500 transition-colors ml-0.5">
                <X className="w-3 h-3" />
              </button>
            </div>
          ))}
        ...
        <div className="flex items-center gap-2">
          <input
            type="text" value={nieuwKostengroep}
            onChange={e => setNieuwKostengroep(e.target.value)}
            onKeyDown={e => {
              if (e.key === 'Enter' && nieuwKostengroep.trim() && !kostengroepen.includes(nieuwKostengroep.trim())) {
                setKostengroepen(prev => [...prev, nieuwKostengroep.trim()]); setNieuwKostengroep('')
              }
            }}
            placeholder="bijv. Werktuigen"
            className="w-48 px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-everts/20 focus:border-everts"
          />
          <button
            onClick={() => {
              if (nieuwKostengroep.trim() && !kostengroepen.includes(nieuwKostengroep.trim())) {
                setKostengroepen(prev => [...prev, nieuwKostengroep.trim()]); setNieuwKostengroep('')
              }
            }}
            className="inline-flex items-center gap-1 text-sm text-everts hover:text-everts-dark border border-everts/30 hover:border-everts/60 px-3 py-2 rounded-lg transition-colors"
...
function OfferteTab() {
  const items = [
    { href: '/instellingen/offerte-layout',      icon: '🎨', label: 'Offerte layout',        sub: 'Word-sjablonen met huisstijl en papierindeling' },
    { href: '/instellingen/betalingscondities',  icon: '💳', label: 'Betalingscondities',     sub: 'Termijnschema\'s met % van de aanneemsom' },
    { href: '/instellingen/algemene-voorwaarden',icon: '📋', label: 'Algemene Voorwaarden',   sub: 'Upload en beheer PDF-documenten met AV' },
    { href: '/instellingen/btw-tarieven',        icon: '📊', label: 'BTW tarieven',           sub: 'Beschikbare BTW-percentages voor offertes' },
  ]

  return (
    <div className="space-y-4">
      <p className="text-sm text-slate-500">
        Offerte-instellingen worden centraal beheerd in EVA.
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {items.map(({ href, icon, label, sub }) => (
          <Link
            key={label}
            href={href}
            className="flex items-start gap-3 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl p-4 transition-colors group"
          >
