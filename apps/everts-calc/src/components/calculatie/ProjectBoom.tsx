...
import type { Selectie, Deelproject, Locatie } from '@/lib/types'
...
function InlineForm({ placeholder, onOpslaan, onAnnuleer }: InlineFormProps) {
  ...
  return (
    ...
    >
      ...
      />
      <button type="submit" className="text-xs bg-everts text-white px-2 py-1 rounded hover:bg-everts-dark">✓</button>
      <button type="button" onClick={onAnnuleer} className="text-xs text-slate-400 px-1 py-1 hover:text-slate-600">✕</button>
    </form>
  )
}
      ...
      <div className="flex-1 overflow-y-auto py-2">
                  ...
                  {dpLocaties.map(loc => (
                    <div key={loc.id} className="flex items-center group px-1 py-0.5">
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 p-1 text-slate-300 hover:text-red-500 rounded"
                      >
                        <Trash2 className="w-3 h-3" />
                      </button>
                    </div>
        ...
        {deelprojecten.length === 0 && !toevoegen && (
          <div className="px-4 py-6 text-center">
            <p className="text-xs text-slate-400 mb-3">Nog geen structuur aangemaakt</p>
            <button
              onClick={() => setToevoegen({ type: 'deelproject', parent_id: projectId })}
              className="text-xs text-everts hover:underline"
            >
              + Eerste deelproject toevoegen
            </button>
          </div>
        )}
      </div>
