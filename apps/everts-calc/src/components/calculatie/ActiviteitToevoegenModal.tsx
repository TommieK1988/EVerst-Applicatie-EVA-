...
import { Search, X, BookOpen, CheckSquare, Square, Plus, Loader2 } from 'lucide-react'
...
import type { Calculatieregel, Componentregel, Eenheid } from '@/lib/types'
import type { BibliotheekItemVereenvoudigd } from '@/lib/types'
import { haalBibliotheekItemsOp } from '@/app/actions/bibliotheek'
  ...
  return (
    <Dialog.Root open={open} onOpenChange={o => !o && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[85vh] flex flex-col">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
            ...
            <Dialog.Close className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg">
              <X className="w-4 h-4" />
            </Dialog.Close>
          </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {laadt && items.length === 0 && (
                  <div className="flex items-center justify-center py-8 gap-2 text-sm text-slate-400">
                    <Loader2 className="w-4 h-4 animate-spin" /> Recepten laden...
                  </div>
                )}
                {gefilterd.map(item => {
              ...
              {selectie.size > 1 ? (
                ...
                <div className="flex-1 flex items-center justify-center text-center p-6 text-slate-400">
                  <div>
                    ...
                    <p className="text-sm">Selecteer een recept</p>
                    <p className="text-xs mt-1 text-slate-300">Meerdere aanvinken voor bulktoevoeging</p>
                  </div>
                </div>
              )}
