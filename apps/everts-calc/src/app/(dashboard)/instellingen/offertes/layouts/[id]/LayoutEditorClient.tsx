...
import { updateLayout } from '@/app/actions/quote-instellingen'
import { maakSjabloontekst, updateSjabloontekst, verwijderSjabloontekst } from '@/app/actions/sjabloonteksten'
import { TEMPLATE_STANDAARD } from '@/lib/quote-templates/standaard'
import { TEMPLATE_UITGEBREID } from '@/lib/quote-templates/uitgebreid'
import { TEMPLATE_BEKNOPT } from '@/lib/quote-templates/beknopt'
// Directe import (geen dynamic) — LayoutEditorClient is al 'use client'
import WysiwygEditor from '@/components/WysiwygEditor'
import type { WysiwygEditorRef, DocumentStyle } from '@/components/WysiwygEditor'
import type { Sjabloontekst } from '@/app/actions/sjabloonteksten'
...
export default function LayoutEditorClient({ layout, voorbeeldQuoteId, sjabloonteksten: initSjabloonteksten }: Props) {
  const [form, setForm] = useState({
    ...
    accent_kleur: layout.accent_kleur ?? '#0a7a35',
  ...
  const previewUrl = voorbeeldQuoteId
    ? form.docx_template_url
      ? `/api/quotes/${voorbeeldQuoteId}/docx-preview?template_url=${encodeURIComponent(form.docx_template_url)}&bedrijf=${encodeURIComponent(bedrijfJson)}`
      : `/api/quotes/${voorbeeldQuoteId}/preview-html?bedrijf=${encodeURIComponent(bedrijfJson)}`
    : null
...
function WordTemplatePaneel({
  ...
  async function uploadTemplate(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch('/api/docx-templates/upload', { method: 'POST', body: formData })
