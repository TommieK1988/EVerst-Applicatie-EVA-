...
import type { Werkbegroting, WerkbegrotingRegel, WerkbegrotingComponent, WerkbegrotingWijziging, WerkbegrotingBestelling, RelatieRef } from '@/lib/types'
...
export async function zoekRelaties(
  ...
  let query = db
    .from('relaties')
    .select('id, naam, type, email, telefoon')
    .ilike('naam', `%${zoekterm}%`)
    .eq('actief', true)
    .limit(20)
  ...
  if (type) {
    ...
    query = query.in('type', ['leverancier', 'onderaannemer'])
  }

  const { data, error } = await query
  if (error || !data) return []
  return data as RelatieRef[]
}
