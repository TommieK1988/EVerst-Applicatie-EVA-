/**
 * GET /api/quotes/[id]/preview-html
 *
...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (error || !quote) {
      console.error('[preview-html] quote niet gevonden:', { id, error: error?.message })
      return new NextResponse('<p>Offerte niet gevonden</p>', { status: 404, headers: { 'Content-Type': 'text/html' } })
    }

    // Sorteer lines per sectie op volgorde (PostgREST ondersteunt geen nested ordering)
    if (quote.sections) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      quote.sections.forEach((s: any) => {
