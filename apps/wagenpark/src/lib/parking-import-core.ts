...
/**
 * Verwerkt een ULU parkeer-Excel buffer naar de database.
 * Gedeelde logica voor zowel het upload-formulier als de e-mail webhook.
 */
export async function runParkingImport(
  buf: Buffer,
  bestandsnaam: string,
  ...
  try {
    ...
    const { rows: batchRows } = await client.query<{ id: string }>(
      ...
      [bestandsnaam, parsed.rows.length, parsed.periode.start, parsed.periode.eind],
    )
