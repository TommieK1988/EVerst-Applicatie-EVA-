...
import { mockStandaardReparaties } from '@/lib/mock-data'
import type { Project, Projectdeel, Locatie, Registratie, Reparatie, StandardRepair, RegistratieStatus } from '@/lib/types'
...
export default function RegistratieDetail({ projectId, projectdeelId, locatieId, registratieId }: Props) {
  ...
  function startBewerken(rep: Reparatie) {
    ...
    if (rep.standaard_reparatie_id) {
      const std = mockStandaardReparaties.find(r => r.id === rep.standaard_reparatie_id)
  ...
  const gefilterdReparaties = mockStandaardReparaties.filter(r =>
