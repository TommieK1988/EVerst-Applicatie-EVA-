...
import { registratieSchema, type RegistratieFormValues } from '@/lib/validations'
import { createRegistratie } from '@/services/registraties'
import { formatCurrency, formatNumber } from '@/lib/utils'
import FotoUpload from '@/components/registraties/FotoUpload'
import { ONDERDEEL_TYPEN, GEVEL_ZIJDEN } from '@/lib/types'
...
export default function RegistratieFormulier({
  ...
  const onSubmit = async (data: RegistratieFormValues, action: 'save' | 'save_new' | 'save_view' = 'save_view') => {
    ...
    try {
      ...
      if (action === 'save_new') {
        router.push('/registraties/nieuw' + (defaultProjectId ? `?project=${defaultProjectId}` : ''))
      } else if (action === 'save_view') {
        router.push(`/registraties/${result.id}`)
      }
    } catch (error: any) {
