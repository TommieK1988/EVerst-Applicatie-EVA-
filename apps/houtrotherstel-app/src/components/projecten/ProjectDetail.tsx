...
import type { Project, Projectdeel, Reparatie } from '@/lib/types'
    ...
    <div className="space-y-6 pb-20 lg:pb-0">
      ...
      <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 self-start">
            ...
            <Link
              href={`/projecten/${project.id}/bewerken`}
              className="inline-flex items-center gap-2 bg-white border border-slate-300 hover:border-slate-400 text-slate-700 font-medium px-4 py-2.5 rounded-lg text-sm transition-colors"
            >
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
              <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-semibold text-slate-800 text-lg">Rapportage opstellen</h2>
                  <button onClick={() => setShowRapportageModal(false)} className="text-slate-400 hover:text-slate-600">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                <p className="text-sm text-slate-500 mb-6">Wil je de rapportage met of zonder prijzen opstellen?</p>
                <div className="space-y-3">
                  <button
                    onClick={() => { setShowRapportageModal(false); router.push(`/projecten/${project.id}/rapportage?prijzen=1`) }}
                    className="w-full bg-everts hover:bg-everts-dark text-white font-medium py-3 rounded-xl text-sm transition-colors"
                  >
                    Met prijzen
                  </button>
                  <button
                    onClick={() => { setShowRapportageModal(false); router.push(`/projecten/${project.id}/rapportage?prijzen=0`) }}
                    className="w-full bg-white border border-slate-300 hover:border-slate-400 text-slate-700 font-medium py-3 rounded-xl text-sm transition-colors"
                  >
                    Zonder prijzen
                  </button>
                </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-xl border border-slate-200 p-4 text-center">
          <div className="text-2xl font-bold text-slate-700">{projectdelen.length}</div>
          <div className="text-xs text-slate-500 mt-0.5">Projectdelen</div>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4 text-center">
          <div className="text-2xl font-bold text-everts">{openCount}</div>
          <div className="text-xs text-slate-500 mt-0.5">Onderhanden</div>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4 text-center">
          <div className="text-2xl font-bold text-green-600">{gereedCount}</div>
