...
import type { Project, ProjectForm, ProjectFilters, ProjectWithStats } from '@/lib/types'
...
export async function createProject(form: ProjectForm, userId: string): Promise<Project> {
  const { saveProject } = await import('@/lib/local-store')
...
export async function updateProject(id: string, form: Partial<ProjectForm>): Promise<Project> {
  const { getAllProjecten, saveProject } = await import('@/lib/local-store')
...
export async function deleteProject(id: string): Promise<void> {
  ...
  const { getLocalProjecten } = await import('@/lib/local-store')
