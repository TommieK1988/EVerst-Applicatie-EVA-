...
import { mockProfile, mockRegistraties, mockProjecten } from '@/lib/mock-data'

export const metadata: Metadata = { title: 'Dashboard' }

export default function DashboardPage() {
  const profile = mockProfile
  const registraties = mockRegistraties
  const projecten = mockProjecten.filter(p => p.status === 'onderhanden')
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900">
            Goedemorgen, {profile.full_name.split(' ')[0]} 👋
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between p-5 border-b border-slate-100">
            ...
            <Link href="/registraties" className="text-sm text-everts hover:underline">
              Alles bekijken
            </Link>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between p-5 border-b border-slate-100">
            ...
            <Link href="/projecten" className="text-sm text-everts hover:underline">Alles</Link>
          </div>
          <div className="divide-y divide-slate-100">
            {projecten.map((p) => (
              <Link
      ...
      {afgekeurdCount > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <Link href="/registraties?status=afgekeurd" className="text-xs text-red-700 font-medium hover:underline mt-1 inline-block">
