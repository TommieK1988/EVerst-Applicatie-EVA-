...
export async function maakQuoteVanuitProjectMetImport(params: {
  ...
  const { data: quote, error } = await supabase
    ...
    .insert({
      ...
      meerwerk_regel_id: params.meerwerkRegelId ?? null,
    })
    .select('id')
    .single()
  ...
  // Meerwerk-offerte: schrijf het quote-id terug op de regel zodat de Meerwerk-tab
  // "Open offerte" toont en de status naar 'offerte_verstuurd' kan.
  if (params.meerwerkRegelId) {
    try {
      ...
      await admin
        ...
        .eq('id', params.meerwerkRegelId)
    } catch { /* best-effort: koppeling is niet kritisch voor het aanmaken */ }
  }
