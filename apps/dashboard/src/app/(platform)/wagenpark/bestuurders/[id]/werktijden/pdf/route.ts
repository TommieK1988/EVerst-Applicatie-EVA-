...
export async function GET(
  ...
  const [gegevens, naamRijen, bedrijf] = await Promise.all([
    laadWerktijdGegevens(periode.van, periode.tot, id),
    pgQuery<{ naam: string }>(
      `select volledige_naam as naam from public.ulu_users where id::text = $1`,
      [id],
    ),
    // Afzender is de organisatie, niet de werkmaatschappij van de bestuurder:
    // dit is een uitdraai ván Everts, en `medewerkers.werkmaatschappij_id` is
    // bij vrijwel iedereen toch leeg.
    laadPdfAfzender(),
  ])
