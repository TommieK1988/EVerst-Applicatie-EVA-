...
/**
 * Start of hervat de kwaliteitsronde die aan deze actie hangt, en stuurt door naar het
 * uitvoerscherm.
 *
 * Autorisatie exact zoals `/m/taken/[taakId]/formulier`: de admin-client passeert de RLS op
 * `tasks` (die laat alleen platform-gebruikers door), dus de afscherming gebeurt hier zelf — je
 * mag alleen een ronde openen van een actie die aan jou is toegewezen.
 */
export default async function MobielTaakKwaliteitPage({
  params,
}: {
  ...
  const res = await startInspectieVoorTaak(taakId)
  if (!res.ok) {
    return (
      <div style={{ padding: 24, color: 'var(--fg-muted)', fontSize: 14, lineHeight: 1.5 }}>
        De kwaliteitsronde kon niet worden gestart: {res.error}
      </div>
    )
  }

  redirect(`/m/kwaliteit/${res.id}`)
}
