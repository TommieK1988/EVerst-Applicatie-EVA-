...
const PAGE_HELP: Array<[RegExp, PageHelp]> = [
  ...
  [/^\/instellingen\/offerte-layout\/[^/]+$/, {
    ...
    sections: [
      ...
      { title: 'Structuurboom met opgetelde totalen ({#boom})', body: 'Wil je de hiërarchie nabootsen — niveau 3 binnen niveau 2 binnen niveau 1 — met totalen die van onder naar boven optellen, gebruik dan de geneste boom-loop. Opbouw: {#boom} (niveau 1) → daarbinnen {#kinderen} (niveau 2) → daarbinnen nog een {#kinderen} (niveau 3) → daarbinnen {#regels} voor de regels. Sluit elke loop op een eigen regel: {/regels}, {/kinderen}, {/kinderen}, {/boom}. Toon per groep {display_naam} als kop. Voor het totaal: {subtotaal_incl} = OPGETELD (eigen regels + alle onderliggende niveaus, dit wil je bij "Totaal Niveau X"); {subtotaal} = alleen de eigen regels van die groep. Handig: {#heeft_regels}…{/heeft_regels} toont het regelblok alleen bij groepen met eigen regels, {#heeft_kinderen}…{/heeft_kinderen} de totaal-regel alleen bij groepen met subgroepen. Let op: combineer {#boom} niet met {#normale_secties} in dezelfde lay-out, anders komen groepen dubbel in de offerte.' },
