...
const PAGE_HELP: Array<[RegExp, PageHelp]> = [
  ...
  [/^\/wagenpark\/werktijden$/, {
    ...
    sections: [
      ...
      { title: 'Een dag controleren', body: 'Klik op een regel. Het zijpaneel zet naast elkaar wat het rooster zei, wat de auto deed en wat er die dag geboekt is, per uursoort. Daarmee zie je meteen of het signaal ergens op slaat: acht uur geschreven terwijl de auto om 15:00 wegreed is iets om te bespreken, zes uur plus twee uur verlof is verklaard.' },
      { title: 'Afvinken', body: 'In het paneel kies je Verklaard (geen actie nodig) of Bespreken (meenemen in een gesprek). Een toelichting is optioneel maar handig — hij wordt met je naam en het tijdstip vastgelegd. Beide halen de dag uit je werklijst. Vergist? Met "Terug naar te controleren" zet je hem terug.' },
      { title: 'Verklaarde dagen blijven staan', body: 'Vink je een dag af als Verklaard, dan verdwijnt hij niet: de afwijking wordt doorgestreept getoond en telt nergens meer in mee — niet in de drie totalen bovenaan, niet in het weeksubtotaal en niet in de samenvatting per medewerker. Zo blijft zichtbaar wat er is weggestreept en waarom. In de samenvatting staan die minuten in een eigen kolom Verklaard. Wil je puur je werklijst, zet dan het vinkje "Verberg afgehandelde dagen" aan.' },
