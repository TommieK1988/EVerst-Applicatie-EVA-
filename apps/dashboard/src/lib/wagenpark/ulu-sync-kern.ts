/**
 * Eén ULU-synchronisatieronde: voertuigen, bestuurders en ritten ophalen en
 * wegschrijven, gevolgd door de compliance-controle.
 *
 * Staat los van de server-action zodat de nachtelijke cron hem ook kan
 * aanroepen — een action begint met `vereisRecht` en een cron heeft geen
 * ingelogde gebruiker. Wat hier staat draait dus ZONDER rechtencontrole: roep
 * het alleen aan vanuit een action die zelf gecontroleerd heeft, of vanuit een
 * cron-route achter `CRON_SECRET`.
 *
 * LET OP — de ULU API geeft maar ongeveer een week terug en negeert
 * datumfilters (zie packages/wagenpark-core/src/ulu-api/client.ts). Een week
 * niet draaien is dus permanent dataverlies; daarom hangt hier een dagelijkse
 * cron aan. Oudere periodes komen alleen via de Excel-import binnen.
 */
import 'server-only'
 ...
 */
export async function voerUluSync(
  periodeStart?: string,
  periodeEind?: string,
  ...
  try {
    ...
    try {
      const cRes = await voerComplianceUit()
