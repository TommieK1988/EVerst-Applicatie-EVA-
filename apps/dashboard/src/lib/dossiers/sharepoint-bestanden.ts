...
const LEEG: DossierSharePointData = { geconfigureerd: false, status: null, mapUrl: null, bestanden: [] }
...
export async function getDossierSharePointBestanden(dossierId: string): Promise<DossierSharePointData> {
  ...
  if (status !== 'gematcht' || !spItemId || !spDriveId) {
    try {
      ...
      return { geconfigureerd: true, status: null, mapUrl: null, bestanden: [] }
    }
  }

  if (status !== 'gematcht' || !spItemId || !spDriveId) {
    return { geconfigureerd: true, status, mapUrl, bestanden: [] }
  }

  try {
    const bestanden = await listFolderChildren(spDriveId, spItemId)
    return { geconfigureerd: true, status: 'gematcht', mapUrl, bestanden }
  } catch {
    return { geconfigureerd: true, status: 'gematcht', mapUrl, bestanden: [] }
  }
}
...
export async function koppelDossierMapViaLink(dossierId: string, shareLink: string): Promise<DossierSharePointData> {
  const m = await resolveShareLink(shareLink)
  if (m.status !== 'gematcht' || !m.driveId || !m.itemId) {
    return { geconfigureerd: true, status: 'niet_gevonden', mapUrl: null, bestanden: [] }
  }
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const supabase = createAdminClient() as any
  await supabase
  ...
  try {
    ...
    return { geconfigureerd: true, status: 'gematcht', mapUrl: m.webUrl ?? null, bestanden }
  } catch {
    return { geconfigureerd: true, status: 'gematcht', mapUrl: m.webUrl ?? null, bestanden: [] }
  }
}
