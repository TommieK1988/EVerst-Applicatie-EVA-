...
export type MeerwerkBatchResultaat = {
  /** Goedgekeurd meerwerk excl. btw per dossier-id. Dossiers zonder meerwerk ontbreken. */
  perDossier: Map<string, number>
  /**
   * Dossiers waarvan het regiedeel niet is meegerekend omdat de limiet werd bereikt.
   * De aanroeper hoort het totaal dan als benadering te tonen.
   */
  regieOvergeslagen: string[]
}

/**
 * Goedgekeurd meerwerk voor een hele set dossiers in één keer — voor objecttotalen.
 *
 * Eén query op `meerwerk_regels` in plaats van N. De regie-/geboekte-kosten-regels zijn
 * het dure deel: die vereisen per dossier een `getServicedeskRegie()`-aanroep richting
 * Bouw7. Daarom wordt dat alleen gedaan voor dossiers die zúlke goedgekeurde regels
 * daadwerkelijk hebben, en niet vaker dan `regieLimiet` keer. Wat daarboven valt komt
 * terug in `regieOvergeslagen` zodat de UI eerlijk "≈" kan tonen in plaats van een te
 * laag getal als exact te presenteren.
 */
export async function getGoedgekeurdMeerwerkExclBatch(
  dossierIds: string[],
  regieLimiet = 10,
): Promise<MeerwerkBatchResultaat> {
  const perDossier = new Map<string, number>()
  const regieOvergeslagen: string[] = []
  if (!dossierIds.length) return { perDossier, regieOvergeslagen }
  ...
  const regels = (data ?? []) as MeerwerkRegel[]
  if (!regels.length) return { perDossier, regieOvergeslagen }

  const perDossierRegels = new Map<string, MeerwerkRegel[]>()
  for (const r of regels) {
    if (!perDossierRegels.has(r.dossier_id)) perDossierRegels.set(r.dossier_id, [])
    perDossierRegels.get(r.dossier_id)!.push(r)
  }

  const heeftRegie = (r: MeerwerkRegel) =>
    (r.afrekenwijze === 'regie' || (r.is_stelpost && r.stelpost_grondslag === 'geboekte_kosten'))
    && !!r.bewakingscode

  const regieDossiers = [...perDossierRegels.entries()]
    .filter(([, rs]) => rs.some(heeftRegie))
    .map(([id]) => id)

  // Regie ophalen tot de limiet; de rest wordt gemeld, niet stilzwijgend op 0 gezet.
  const teDoen = regieDossiers.slice(0, regieLimiet)
  regieOvergeslagen.push(...regieDossiers.slice(regieLimiet))

  const regiePerDossier = new Map<string, Map<string, number>>()
  for (const id of teDoen) {
    try {
      const regie = await getServicedeskRegie(id)
      const perCode = new Map<string, number>()
      for (const r of regie.regels) {
        if (r.uitgesloten || !r.bewakingscode) continue
        perCode.set(r.bewakingscode, (perCode.get(r.bewakingscode) ?? 0) + (r.verkoopBedrag || 0))
      }
      regiePerDossier.set(id, perCode)
    } catch {
      // Bouw7 onbereikbaar voor dit dossier: behandel als "niet meegerekend".
      regieOvergeslagen.push(id)
    }
  }

  for (const [dossierId, rs] of perDossierRegels) {
    const perCode = regiePerDossier.get(dossierId) ?? new Map<string, number>()
    const som = rs.reduce((t, r) => t + effectiefExcl(r, perCode), 0)
    perDossier.set(dossierId, rond(som))
  }

  return { perDossier, regieOvergeslagen }
}
