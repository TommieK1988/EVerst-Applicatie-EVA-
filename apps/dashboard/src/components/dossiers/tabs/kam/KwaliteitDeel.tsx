...
/**
 * Kwaliteitscontrole en VCA binnen de KAM/VGM-tab.
 *
 * `vcaAan` is de dossier-toggle `vca`. Staat die uit, dan blijft alleen de
 * kwaliteitscontrole over: de VCA-administratie (acties, diploma's) is dan niet van
 * toepassing op dit dossier. De tab zelf blijft wel bereikbaar — daar hangen ook de
 * oplevering en de formulieren onder, en die staan los van VCA.
 */
export default async function KwaliteitDeel({
  dossierId,
  vcaAan,
}: {
  dossierId: string
  ...
  // Niets van de VCA-kant ophalen wanneer de toggle uitstaat; dat scheelt drie queries
  // op elk dossier zonder VCA.
  const [bemensing, acties] = vcaAan
    ? await Promise.all([getVcaBemensing(dossierId), getVcaActies(dossierId)])
    : [[] as VcaBemensingRij[], [] as VcaActie[]]
  ...
  return (
    <div style={{ padding: 'var(--page-pad-y, 28px) var(--page-pad-x, 32px)', maxWidth: 860 }}>
      <h2 style={{ margin: '0 0 24px', fontSize: 18, fontWeight: 700 }}>Kwaliteit &amp; VCA</h2>
      ...
      {!vcaAan ? (
        <p style={{ fontSize: 13, color: 'var(--text-muted)' }}>
          De VCA-toggle staat uit op dit dossier. Zet hem aan op de Informatie-tab om
          VCA-acties en de diploma&apos;s van de betrokken medewerkers hier te volgen.
        </p>
      ) : (
        <>
