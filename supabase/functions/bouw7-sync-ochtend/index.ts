/**
 * Supabase Edge Function: bouw7-sync-ochtend
 *
 * Ochtend-run van de volledige Bouw7-sync — 06:30 NL (zie supabase/config.toml).
...
Deno.serve(async (_req) => {
  ...
  if (!appUrl || !cronSecret) {
    ...
    console.error('[bouw7-sync-ochtend]', msg)
  ...
  try {
    ...
    if (res.ok) {
      console.log(`[bouw7-sync-ochtend] Geslaagd — duur: ${data.duur_ms ?? '?'}ms`)
    } else {
      console.error('[bouw7-sync-ochtend] App meldde fout:', data)
    }
    ...
    console.error('[bouw7-sync-ochtend] Netwerkfout:', msg)
    return new Response(JSON.stringify({ ok: false, error: msg }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    })
